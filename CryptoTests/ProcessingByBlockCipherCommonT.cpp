...
TEST(ProcessingByBlockCipherCommonT, AesRoundsKeys) {
    ...
    std::unique_ptr< uint64_t> output(new uint64_t[2]);
    AesEncryptBlock((uint64_t*)((Aes128State*)((BlockCipherState*)handle)->state)->roundsKeys, 9, (uint64_t*)input, output.get());

    std::unique_ptr< uint64_t> output2(new uint64_t[2]);
    AesDecryptBlock((uint64_t*)((Aes128State*)((BlockCipherState*)handle)->state)->roundsKeys, 9, output.get(), output2.get());
}
